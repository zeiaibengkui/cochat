<template>
    <div class="chat-panel" :style="{ width: chatWidth + 'px' }">
        <div v-if="nav.currentPath.value.length" class="path-bar px-3 py-1">
            <div class="d-flex ga-1 align-center flex-wrap">
                ...
                <template v-for="(nodeId, i) in nav.currentPath.value" :key="nodeId">
                    ...
                    <VChip
                        :color="nodeId === nav.focusedNodeId.value ? 'primary' : undefined"
                        :variant="nodeId === nav.focusedNodeId.value ? 'flat' : 'outlined'"
                        size="x-small"
                        label
                        @click="nav.setFocused(nodeId)"
                    >
        ...
        <div ref="msgContainer" class="msg-container">
            ...
            <div
                v-else-if="!pathMsgs.length && !streaming"
                class="text-center text-medium-emphasis pa-4"
            >
                Focus a node and send a message
            </div>
            <div
                v-for="msg in pathMsgs"
                :key="msg.id"
                class="msg-wrapper"
                ...
                <div v-else class="msg-bubble">
                    ...
                    <template v-else>
                        <div class="msg-role">
                            ...
                            <span v-if="msg.property?.edited" class="text-warning text-caption ml-1"
                                >(edited)</span
                            >
                        </div>
                ...
                <div class="msg-bubble">
                    ...
                    <div class="msg-content">
                        <MarkdownRender
                            ...
                            html-policy="escape"
                            :katex="true"
                            :content="mdPreprocess(streaming)"
                            :final="false"
...
function startResize(e: MouseEvent) {
    ...
    function move(ev: MouseEvent) {
        chatWidth.value = Math.min(MAX_CHAT, Math.max(MIN_CHAT, startW + (startX - ev.clientX)));
    }
    function up() {
...
function isOwn(msg: graphNode) {
    return msg.author === userName.value && msg.property?.role === "user";
}
...
const pathMsgs = computed(() => {
    ...
    return path
        .slice(1)
        .map((id: number) => nav.tree.value?.nodes.find((n: graphNode) => n.id === id))
...
const graphTopic = computed(() => {
    const g = nav.graphs.value.find(
        (gr: { id: number; property: { topic: string } | null }) => gr.id === nav.graphId.value,
    );
...
function stop() {
    ...
    abort = null;
    if (streaming.value) {
        $fetch<number>("/api/node", {
            method: "POST",
            body: {
                parent: nav.currentPath.value.at(-1),
                author: store.selectedModel,
                property: { text: streaming.value, role: "assistant" },
            },
        }).then((id) => {
            if (id) nav.setFocused(id);
            nav.refreshTree();
        });
    }
    streaming.value = "";
    ...
    try {
        ...
        try {
            const errId = await $fetch<number>("/api/node", {
                method: "POST",
                body: {
                    parent: nav.focusedNodeId.value,
                    author: "system",
                    property: { text: "Error: " + errMsg, role: "assistant" },
                },
            });
...
function buildCtx(userText: string) {
    ...
    for (const n of pathMsgs.value) {
        ...
        msgs.push({
            role: (n.property?.role as "user" | "assistant") ?? "user",
            content: n.property?.text ?? "",
        });
    }
