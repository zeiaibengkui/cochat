<template>
	<VCard class="node-card" :class="{ 'root-node': depth === 0, 'deleted-node': isDeleted }">
		<VCardItem>
			<template v-if="editing">
				...
				<div class="d-flex ga-2 mt-2">
					...
					<VBtn size="small" variant="text" @click="cancelEdit">
						Cancel
					</VBtn>
				</div>
			</template>
			...
			<template v-else>
				<VCardTitle class="text-body-1 pa-0" :class="{ 'text-decoration-line-through text-medium-emphasis': isDeleted }">
					{{ nodeText }}
					<VChip v-if="isEdited && !isDeleted" size="x-small" label class="ml-1" color="warning">
						edited
					</VChip>
				</VCardTitle>
		...
		<VCardActions v-if="!editing && !isDeleted">
			...
			<VBtn size="small" variant="text" prepend-icon="mdi-pencil"
				:disabled="!isAuthor" @click="startEdit">
				Edit
			</VBtn>
			<VSpacer />
			<VBtn size="small" variant="text" color="error"
				prepend-icon="mdi-delete-outline"
				:disabled="!isAuthor" :loading="deleting"
		...
		<VExpandTransition>
			<VCardText v-if="confirmDelete" class="bg-surface-variant">
				...
				<div class="d-flex ga-2">
					<VBtn size="small" color="error" variant="flat" :loading="deleting" @click="deleteNode">
						Confirm
					</VBtn>
					<VBtn size="small" variant="text" @click="confirmDelete = false">
						Cancel
					</VBtn>
				</div>
			</VCardText>
		...
		<div v-if="children?.length" class="children-container pl-6">
			<node v-for="child in children" :key="child.id"
				:id="child.id" :depth="depth + 1"
				@deleted="refresh()" />
		</div>
	</VCard>
</template>
...
const { data: children, refresh } = await useFetch<NodeData[]>(
	"/api/children",
	{ params: { parent: props.id } },
);

const { data: nodeData, refresh: refreshNode } = await useFetch<NodeData>(
	"/api/node",
	{ params: { id: props.id } },
);
...
async function saveNode() {
	...
	try {
		await $fetch("/api/modNode", {
			method: "POST",
			body: { id: props.id, property: { text: editText.value, role: nodeProp.value?.role ?? "user" } },
		});
		await refreshNode();
		editing.value = false;
	} catch { /* handled */ }
